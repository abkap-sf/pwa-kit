...
const registerEventListeners = (siteId, slasToken, basketId, domainUrl) => {
    ...
    return [{event: 'onEmbeddedMessagingReady', handler: onReadyHandler}]
}
...
const initEmbeddedMessaging = (messaging, orgId, esdName, esdUrl, scrt2Url) => {
    try {
        if (onClient && messaging && messaging?.embeddedservice_bootstrap?.settings) {
            ...
            messaging.embeddedservice_bootstrap.init(orgId, esdName, esdUrl, {scrt2URL: scrt2Url})
        }
    } catch (err) {
        ...
        const setStateFromEvent = (event) => {
            ...
            if (loaded) {
                ...
                if (!isMiawInitialized) {
                    miawEventListeners = registerEventListeners(
                        siteId,
                        slasToken,
                        basketId,
                        domainUrl
